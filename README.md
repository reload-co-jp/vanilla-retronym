...
| `name`         | string   |    ○ | レトロニム                             |
| `aliases`      | string[] |    - | 同じものを指す別名。検索対象になる     |
...
- `etymology`：語の成り立ち（どの語に何を付けたか、和製英語か等）と、確認できれば初出・広まった時期。出典で確認できない時期は書かない
...
- 語源・呼称の由来（何と区別するために、いつ頃その名前が使われ始めたか）は、それを明記した出典で確認する。辞書（コトバンクなど）、語源・呼称を扱う文献、各記事の「呼称」「語源」の節などを使う
- 呼称の由来を明記した出典が見つからない場合は、`status` を `disputed` / `candidate` にし、本文でも断定しない。語源に諸説ある場合は主な説を併記する
- `sources` の URL は実際にアクセスできることを確認してから登録する
